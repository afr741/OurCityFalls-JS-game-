function Main() {
    ...
    this.showInstructions = function() {
      gameUI.writeText('Controls: Use arrow keys to move/jump, shift to run, s to shoot', 30, 30);
      gameUI.writeText('Tip: Jumping while running makes you jump higher', 30, 60);
    };
  
    this.renderMap = function() {
      //setting false each time the map renders so that elements fall off a platform and not float around
  ...
  /*--------------------------------------------------------------------------------------*/
    this.collisionCheck = function(objA, objB) {
      ...
      // if the x and y vector are less than the half width and half height, then we must be inside the object, causing a collision
