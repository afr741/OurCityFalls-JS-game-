function MarioGame() {
    ...
    this.checkElementPowerUpCollision = function(element) {
      for (var i = 0; i < powerUps.length; i++) {
        ...
        if (collisionDirection == 'l' || collisionDirection == 'r') {
          powerUps[i].velX *= -1; //change direction if collision with any element from the side
        } else if (collisionDirection == 'b') {
    ...
    this.updateMario = function() {
      ...
      if (keys[38] || keys[32]) {
        ...
        if (!mario.jumping && mario.grounded) {
          ...
          if (mario.frame == 0 || mario.frame == 1) {
            mario.frame = 2; //right jump
          } else if (mario.frame == 8 || mario.frame == 9) {
            mario.frame = 3; //left jump
          }
      ...
      if (keys[88] && mario.type == 'fire') {
        ...
        if (!bulletFlag) {
          ...
          setTimeout(function() {
            bulletFlag = false; //only lets mario fire bullet after 800ms.
          }, 800);
        }
      }

     /* if(keys[67] && mario.type == 'grenade'){
        //c key
        if(!grenadeFlag){
          grenadeFlag = true;
          ...
          grenade.init(mario.x, mario.y, direction);
    ...
    this.levelFinish = function(collisionDirection) {
      //game finishes when mario slides the levelWinFlag and collides with the ground
