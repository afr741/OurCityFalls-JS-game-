...
class Vector { // class to determine x, and y for the player such as position and velocity.
    constructor(x,y){
        this.set(x,y);
...
class Level {
    constructor(){
        ...
        this.Entity = new Set();
        this.brickCollision = null;
        }

        setCollisionGrid(grid) {
            this.brickCollision = new Collision(grid);
...
class Collision {
    constructor(brickGrid){
        this.bricks =new BrickConverter(brickGrid);
    }
...
function drawLevel(name) { // draw Level function to read Json files 
    ...
    .then(function([levelDetails,pixels]){
        ...
        return mergedTiles.concat(layerSpec.bricks);
        }, []);
...
function createCollisionGrid(bricks, patterns) {
    const grid = new Grid();
    for (const {brick,x,y} of expandTiles(bricks, patterns)) {
        grid.set(x, y, { type: brick.type });
    }
    return grid;
}
function createBackgroundGrid(bricks, patterns) {
    const grid = new Grid();
    for (const {brick,x,y} of expandTiles(bricks, patterns)) {
        grid.set(x, y, { name: brick.name });
    }
    return grid;
}
...
function expandTiles(tiles, patterns){  // Iterate through the Json file reading the dimensions of the x's and
    const expandedTiles = [];
    function walkTiles(bricks, offsetX, offsetY) {
        for(const brick of bricks){
            for (const{x,y} of expandRanges(brick.ranges)) {
                const deriveX = x + offsetX;
                const deriveY = y + offsetY;
                if(brick.pattern){
                const bricks = patterns[tile.pattern].bricks;
                walkTiles(bricks, deriveX, deriveY);
            }
            else {
                expandedTiles.push({
                    brick,
...
function createbgLayer (level,bricks, pixels){   // create the background layer first
   
    var resolver = new BrickConverter(bricks);
        ...
        if (col){
            col.forEach((brick,y) => {
                if (pixels.animations.has(brick.name)) {
                    pixels.drawAnim(brick.name, context, x - startIndex, y, level.totalTime);
                }
                else {
...
function createPixelLayer(Entity, width = 64, height = 64){
